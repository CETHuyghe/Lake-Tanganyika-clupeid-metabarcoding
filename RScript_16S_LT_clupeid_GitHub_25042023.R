...
# Check how many unassigned OTUs per species
LM_samples <- rownames(metadata[which(metadata$Species == "Limnothrissa_miodon"),])
counts_LM <- counts[ ,colnames(counts) %in% LM_samples ]
counts_LM = counts_LM[rowSums(counts_LM) > 0,]
LM_unass <- rownames(counts_LM[ rownames(counts_LM) %in% unnas_OTUs, ])

ST_samples <- rownames(metadata[which(metadata$Species == "Stolothrissa_tanganicae"),])
counts_ST <- counts[ ,colnames(counts) %in% ST_samples ]
counts_ST = counts_ST[rowSums(counts_ST) > 0,]
ST_unass <- rownames(counts_ST[ rownames(counts_ST) %in% unnas_OTUs, ])
...
nmds.plot <- ggplot(site.scrs, aes(x=NMDS1, y=NMDS2))+ #sets up the plot
  geom_point(aes(NMDS1, NMDS2, fill = factor(Species),colour = factor(Species), shape = factor(Loc_season)), size = 8)+ #adds site points to plot, shape determined by Landuse, colour determined by Management
  coord_fixed()+
...
order_barplot <- c("North_Dry", "North_Wet", "Center_Wet","South_Wet")
metadata3$LocSeas <- factor(metadata3$LocSeas, levels=order_barplot)
...
adonis2(BrCu.uvira~Species, data=metadata_uvira, permutations = 999, p.adjust.m = "bonferroni") # Does not make sense, can be seasonal signal
...
# Mean diversity
metadata_ST <- metadata4[ rownames(metadata4) %in% ST_samples , ]
mean(metadata_ST$Shannon)
mean(metadata_ST$Chao1)

metadata_LM <- metadata4[ rownames(metadata4) %in% LM_samples , ]
